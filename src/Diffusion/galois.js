...
const gMul = (a, b) => {
  if (a === 0 || b === 0) return 0;
  const val = (logTable[a] + logTable[b]) % 255;
  return antilogTable[val];
};
...
const mds_aes_mds =[
    [0x01, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x10, 0x02, 0x1e ],
...
const mdsMultiply = mds => row => {
  const ans = [];
  if(row.length !== mds[0].length)  
    throw Error ("row size should be equal to the size of row in mds");
  for(let r of mds) {
    let cur = 0;
    for(let i=0; i < row.length; ++i) {
      cur = gAdd(cur, gMul(r[i], row[i]));
    }
    ans.push(cur);
  }
  return ans;
...
export const aesGMul = mdsMultiply(aes_mds);
export const mdsAesGmul = mdsMultiply(mds_aes_mds);